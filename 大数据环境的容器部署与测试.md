...
1. 将上述脚本保存为 `setup_hadoop.sh`
2. 授予执行权限：`chmod +x setup_hadoop.sh`
3. 执行脚本：`./setup_hadoop.sh`

## 注意事项

- 已按修改了 Docker 镜像源配置方式，使用了 `sudo nano /etc/docker/daemon.json <<EOF` 的方法
- 使用了指定的镜像源 `https://docker.1ms.run` 和 `https://docker.xuanyuan.me`
- 脚本在使用 `nano` 编辑器，可能需要保存文件（按 Ctrl+O，然后 Enter）并退出（按 Ctrl+X）
